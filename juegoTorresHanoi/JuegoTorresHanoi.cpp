...
void JuegoTorresHanoi<t>::realizarTodosLosMovimientos() {
    ...
    // Iterar sobre cada movimiento y mostrar el estado de las torres después de cada movimiento
    for (int i = 1; i <= 15; ++i) {
        ...
        contador++;
        dibujar(); // Mostrar el estado de las torres después de cada movimiento
    }
    ...
    do {
        ...
        switch (opc) {
            ...
            case 1 : {
                ...
                auto end = std::chrono::steady_clock::now(); // Fin del temporizador
                std::chrono::duration<double> elapsed_seconds = end - start; // Tiempo transcurrido
                std::cout << "Tiempo de iteración: " << elapsed_seconds.count()
